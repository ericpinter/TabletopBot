...
impl ArithmeticParser{
    ...
    fn full_eval(&mut self,expression: Pairs<Rule>) -> Result<Calculation,String> {
        PREC_CLIMBER.climb(
            expression,
            |pair: Pair<Rule>| match pair.as_rule() {
                ...
                Rule::Ternary => {// in the form t?x:y. x is the value of this statement if t is non-zero, y is the value if it is zero
                    ...
                    let t = self.full_eval(Pairs::single(stmt.next().unwrap()))?.output;
                ...
                Rule::Norm =>{// in the form xdy e.g. 3d6
                    let mut stmt = pair.into_inner();
                    let x = self.full_eval(Pairs::single(stmt.next().unwrap()))?.output.as_num()?;
                ...
                Rule::Keep =>{//e.g. 3d6k2
                    ...
                    let x = self.full_eval(Pairs::single(stmt.next().unwrap()))?.output.as_num()?;
                    let y = self.full_eval(Pairs::single(stmt.next().unwrap()))?.output.as_num()?;
                    let k = self.full_eval(Pairs::single(stmt.next().unwrap()))?.output.as_num()?;
                ...
                Rule::FrontEq =>{
                    ...
                    let result = self.full_eval(val_group.into_inner());
                ...
                Rule::Repeat => {
                    if self.repeat_count>100 {Ok(Calculation::empty_str())}
                    else {
                        let mut stmt = pair.into_inner();
                ...
                _ => {eprintln!("failed at {}", pair.to_string()) ; unreachable!()}
            },
